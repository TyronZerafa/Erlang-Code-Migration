...
'(' ')' ',' '->' ':-' '{' '}' '[' ']' '|' '||' '<-' ';' ':' '#' '.'
'after' 'begin' 'case' 'try' 'catch' 'end' 'pfun' 'fun' 'if' 'of' 'receive' 'when'
'andalso' 'orelse'
'bnot' 'not'
'*' '/' 'div' 'rem' 'band' 'and'
...
type -> binary_type                       : '$1'.
type -> integer                           : '$1'.
type -> 'pfun' '(' ')'                     : {type, ?line('$1'), 'fun', []}.
type -> 'pfun' '(' fun_type_100 ')'        : '$3'.
...
loop([],Result) -> Result;
loop([H|T], Result) ->
	loop(T,loop(H,Result));
loop({_M,A},Result) ->
	[loop_A(A,Result)|Result].

loop_A([], Result) -> Result;
loop_A([{_L,_M,{unbound_var,Var}}|T],Result) ->
	loop_A(T,[{tuple,1,[{atom,1,Var},{var,1,Var}]}|Result]);%%[{{atom,1,Var},{var,1,Var}}|Result]);
loop_A(_A, Result) ->
	io:format("Encountered"),
		Result.
		
c(Line,[]) ->
	{nil,Line};
c(Line,[H|T]) ->
	{cons,Line,H,c(Line,T)}.
