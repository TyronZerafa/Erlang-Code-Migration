...
-export([apply/2,apply/3,spawn/4,spawn_link/4,
	 ...
	 disconnect_node/1]).
-export([spawn/1, spawn_link/1, spawn/2, spawn_link/2, get_result/1]).
-export([yield/0]).
-export([crasher/6]).
-export([fun_info/1,is_portable/1,portable_fun_info/2,portable_fun_info/1]).
...
spawn(N, F) when erlang:is_function(F) ->
    case is_portable(F) of
		false -> erlang:spawn(N, erlang, apply, [F, []]);
		true -> mcode:remote_spawn(N, F, erlang:group_leader(), infinity)
    end;		
...
spawn(N,M,F,A) when erlang:is_atom(N),
    ...
    case catch 
    	%gen_server:call({net_kernel,N},{spawn,M,F,A,erlang:group_leader()},infinity) of
		mcode:remote_spawn(N, M, F, A, erlang:group_leader(), infinity) of 
...
is_portable(Fun) ->
	...
	lists:prefix("-build_eval_fun",erlang:atom_to_list(Name)).
...
fun_info(Fun) when erlang:is_function(Fun) ->
    case is_portable(Fun) of 
	true ->
		portable_fun_info(Fun);
	false ->
		Keys = 
...
get_portable_fun_code([],_) -> [];
get_portable_fun_code([[{M,F,A},C,_D]|_OtherPFuns],{M,F,A}) -> C;
get_portable_fun_code([_|OtherPFuns],{M,F,A}) -> get_portable_fun_code(OtherPFuns,{M,F,A}).

get_portable_fun_calls([],_) -> [];
get_portable_fun_calls([[{M,F,A},_C,D]|_OtherPFuns],{M,F,A}) -> D;
get_portable_fun_calls([_|OtherPFuns],{M,F,A}) -> get_portable_fun_calls(OtherPFuns,{M,F,A}).

portable_fun_info(Fun,Item) ->
	P = erlang:is_portable(Fun),
	if 	
		P == false -> erlang:error(not_portable);
		true ->
			{env,[PFunLst,FunName,ModName]} = erlang:fun_info(Fun, env),
			case Item of
				...
				code -> 
					...
					{code,get_portable_fun_code(PFunLst,{ModName,FunName,A})};
				calls -> 
					{arity,A} = erlang:fun_info(Fun,arity),
					{calls,get_portable_fun_calls(PFunLst,{ModName,FunName,A})};
				type -> {type,portable};
				{dependency_code,Dep} -> 
					{dependency_code,get_portable_fun_code(PFunLst,Dep)};
				{dependency_dep,Dep} -> 
					{dependency_dep,get_portable_fun_calls(PFunLst,Dep)}
			end
	end.
...
portable_fun_info(Fun) ->
	...
	[{module,ModName},{name,FunName},{arity,Arity},{code,get_portable_fun_code(PFunLst,{ModName,FunName,Arity})},{calls,get_portable_fun_calls(PFunLst,{ModName,FunName,Arity})},{type,portable}].
